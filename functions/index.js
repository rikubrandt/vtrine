...
exports.processMedia = functions
  ...
  .storage.object().onFinalize(async (object) => {
    ...
    const outputFilePath = path.join(os.tmpdir(), `cropped-${path.basename(filePath, path.extname(filePath))}${outputExtension}`);
    ...
    try {
      if (isVideo) {
        ...
        if (videoDurationSeconds > 30) {
          console.log(`Skipping video as it exceeds 30 seconds: ${videoDurationSeconds} seconds`);
          return null;
        }
